...
function ESPMoveset.new(State, Assets, specialCooldowns)
    local enabled = State.Toggles.ESPMoveset
    ...
    local function Render(c, frame)
        ...
        local movesetFolder = frame.movesetFolder
        local isReggie = frame.isReggie
        local nextReceiptObj = frame.nextReceiptObj
        local scaleFactor = frame.scaleFactor
        local root2D = frame.root2D
        local uY = frame.uY
        local SPECIAL_COOLDOWNS = specialCooldowns
        local slotY = uY
        ...
        if enabled.Value then
                    ...
                    slotY = uY - slotHeight - m_floor(m_clamp(4 * scaleFactor, 2, 8))
                    hasActiveMoveset = false
        ...
        frame.slotY = slotY
        frame.hasActiveMoveset = hasActiveMoveset
    end
